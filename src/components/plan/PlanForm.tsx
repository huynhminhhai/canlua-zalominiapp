    ...
    const renderContent = () => {
        ...
        return (
            <Box>
                ...
                <Box pt={2}>
                    <div className="space-y-4 pb-[54px]">
                        {data.result.map((item: GoiNangCapType, index) => (
                            <div
                                key={item.id}
                                className={`relative border-[1px] rounded-xl px-5 pb-5 pt-6 bg-white transition-all duration-100 cursor-pointer shadow-sm transform ${selectedPlan === item.id
